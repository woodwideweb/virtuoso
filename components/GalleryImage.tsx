  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full absolute left-0 top-0 bg-gradient-to-b from-transparent via-primary-900/50 to-primary-900/90 flex flex-col justify-end items-start p-8 opacity-0 hover:opacity-100 transition-opacity duration-300 group">
        ...
        <span className="font-medium text-xl text-white translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
          {city}, OH
        </span>
      </div>
    </div>
...
export const MediumImageVertical: React.FC<Props> = ({
  src,
  alt,
  className,
  city,
  ...
  <GalleryImage
    src={src}
    alt={alt}
    className={cx(`row-span-2`, className)}
    city={city}
    year={year}
  />
);

export const MediumImageHorizontal: React.FC<Props> = ({
  src,
  alt,
  className,
  city,
...
export const BigImage: React.FC<Props> = ({
  src,
  alt,
  className,
  city,
